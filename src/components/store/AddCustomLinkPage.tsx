...
import { ArrowLeft, Link as LinkIcon, Image as ImageIcon, X as XIcon } from 'lucide-react'
...
export default function AddCustomLinkPage({ onBack, onSave }: AddCustomLinkPageProps) {
  ...
  const [thumbnailSize, setThumbnailSize] = useState<'big' | 'small' | 'none'>('none')
  ...
  const handleSave = () => {
    ...
    if (!hasError) {
      ...
      onSave(title.trim(), normalizedUrl, thumbnailUrl, thumbnailSize, customIconUrl)
      setIsSaving(false)
    }
  }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-shrink-0 bg-white dark:bg-gray-950 px-6 py-4 border-b border-gray-200 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-base font-semibold">Add Featured Link</h3>
        </div>
      </div>
      ...
      {/* Fixed Footer with Actions and Add Button */}
